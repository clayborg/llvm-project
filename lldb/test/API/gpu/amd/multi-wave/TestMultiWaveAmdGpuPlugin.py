...
class BasicAmdGpuTestCase(AmdGpuTestCaseBase):
    ...
    def test_step_over_preserves_selected_lane(self):
        """Stop multiple waves at one breakpoint, then step a non-first lane in
        wave B while wave A remains stopped. The completed step must select
        the explicitly stepped lane in wave B, keep wave A's original stop
        reason intact, and report the same lane through qC."""
        self.build()
