"""
Test GPU page fault exception detection in IntelGT.

Verifies that invalid global memory access triggers a page fault exception
that is correctly detected and reported by LLDB with appropriate signal
...
class IntelGtPageFaultExceptionTestCase(IntelGtTestCaseBase):
    """Test page fault exception detection."""

    def test_page_fault_exception(self):
        """Test that GPU page fault is detected and reported correctly.

        Kernel writes to nullptr, triggering a page fault. The driver sends
        ZET_DEBUG_EVENT_TYPE_PAGE_FAULT followed by thread stopped events.

        Expected behavior:
        1. GPU process stops with eStopReasonException
        2. At least one GPU thread reports the exception
        3. Stop description contains "Page fault"
        """
        self.build()

        exe = self.getBuildArtifact("invalid_global_access")
        ...
        # Wait for process to stop (loop in case first event is eStateRunning)
        ...
        self.assertEqual(gpu_process.GetState(), lldb.eStateStopped,
                        "GPU process should be stopped at exception")
        ...
        self.assertTrue("Page fault" in stop_desc,
                       f"Stop description should mention 'Page fault', got: '{stop_desc}'")
        ...
        print(f"SUCCESS: Page fault exception detected")
