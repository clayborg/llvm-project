  ...
  def run(self):
    while True:
      ...
      if not self.listener.WaitForEvent(self.event_timeout, event):
        raise Exception('[ProcessStateListener] Timeout occurred waiting for event...')
...
class TestNVIDIAGPUAssert(TestBase):
    ...
    def test_gpu_asserting(self):
        ...
        def onGPUStop(proc):
          ...
          # TODO: Remove this. At some point we won't have this public inititialization
          # stop event, so we can remove this check at that point.
          if "NVIDIA GPU is initializing" in description:
            # Let's continue processing events
            return True
          elif "NVIDIA GPU Thread Stopped by Exception" in description:
            # We stop processing events
            return False
          raise Exception("Unexpected GPU stop reason")
