  ...
  auto drain_once = [&]() -> size_t {
    ...
    for (auto &qe : queued) {
      ...
      case ZET_DEBUG_EVENT_TYPE_THREAD_UNAVAILABLE: {
        ...
        // Also count this toward the expected-stop tally. If a stepped thread
        // reports UNAVAILABLE instead of STOPPED (kernel finished mid-step),
        // the tally would otherwise stay positive and Halt() would never run.
        if (m_expected_stopped_count > 0)
          m_expected_stopped_count--;
        any_change = true;
        break;
