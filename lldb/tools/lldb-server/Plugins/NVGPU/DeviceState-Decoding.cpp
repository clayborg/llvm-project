...
size_t ThreadState::DecodeThreadInfoBuffer(
  ...
  m_thread_nvgpu.GetRegisterContext().InvalidateAllRegisters();
  ...
  if (is_active) {
    ...
    if (m_exception)
      m_thread_nvgpu.SetStoppedByException(*m_exception);
    else if (at_breakpoint)
      m_thread_nvgpu.SetStoppedByBreakpoint();
    else
      m_thread_nvgpu.SetStopped();
  } else {
    m_exception = {};
    m_thread_nvgpu.SetStopped();
  }
