...
TEST_F(DWARFExpressionMockProcessTest, DW_OP_LLVM_piece_end) {
  ...
  ASSERT_TRUE(
      CreateTestContext(&test_ctx, "i386-pc-linux", RegisterValue(reg_value)));

  ExecutionContext exe_ctx(test_ctx.process_sp);
  MockDwarfDelegate delegate = MockDwarfDelegate::Dwarf5();

  // Match the flat expression emitted for a value split across two AMDGPU
  // registers. DW_OP_LLVM_piece_end completes the two-piece composite.
  EXPECT_THAT_EXPECTED(
...
TEST(DWARFExpression, DW_OP_LLVM_piece_end_multiple) {
  EXPECT_THAT_ERROR(
      Evaluate({DW_OP_const1u, 0xaa, DW_OP_piece, 1, DW_OP_LLVM_user,
          ...
          .takeError(),
      llvm::FailedWithMessage(
          "could not evaluate DW_OP_LLVM_user: DW_OP_LLVM_piece_end is only "
          "supported at the end of an expression"));
}
